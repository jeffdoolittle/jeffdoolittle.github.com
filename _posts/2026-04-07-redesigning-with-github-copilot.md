...
Instead of jumping straight into the stylesheet and guessing my way toward something I liked, I asked Copilot to generate three distinct design directions as standalone HTML files I could open in a browser and compare side by side. Within minutes I had three fully rendered prototypes — a modern professional look with a dark navy header and amber accents, a minimal clean option with lots of whitespace, and a warm editorial feel with off-white tones.

I chose the first one. The others got deleted. The whole exploration took maybe twenty minutes, and I went into implementation already knowing exactly what I wanted. That's not how this usually goes.

## What Actually Changed

The implementation touched 17 files. The main stylesheet nearly doubled — from 615 lines to over 1,200. But the most important change wasn't volume, it was architecture. Copilot migrated all the hardcoded SCSS color variables to CSS custom properties, which meant the entire color system was now themeable. That turned out to matter more than I realized at the time.

The visual result was significant: a full-bleed hero on the homepage, a card-based post grid instead of a flat list, a wider layout, custom fonts, and a structured footer. The site finally looked like something I'd actually designed rather than something I'd inherited from a template.

It also patched four gems with known security vulnerabilities while it was at it. That's not glamorous work, but it's the kind of thing that quietly accumulates on a personal project when nobody's watching.

## The Deployment Rabbit Hole

Nothing ever goes completely smoothly. Updating the gems broke the Netlify build — the Ruby version Netlify was using didn't play well with one of the updated packages. We went through three iterations to find the right combination: wrong Ruby version, then wrong build image, then finally the right pin. Each attempt was small, targeted, and committed with a clear message explaining what we tried and why.
...
Prototype-first design, systematic issue tracking, meaningful commit messages, automated security awareness — these aren't new ideas. But they're easy to skip on a personal project when it's just you and there's no team expecting you to do it right. Copilot enforced a discipline I probably would have cut corners on working alone, and the result is a codebase that's genuinely easier to maintain.
